(function (global, factory) {
    ...
    class Tile$1 extends Entity$1 {
        ...
        constructor(config) {
            ...
            // @ts-ignore
            this.flags.tileMech = GW.flag.from(TileMech, this.flags.tileMech, config.flags);
            if (config.effects) {
                Object.entries(config.effects).forEach(([key, info]) => {
                    if (info) {
                        if (typeof info === 'string') {
                            if (GW.effect.effects[info]) {
                                this.effects[key] = GW.effect.effects[info];
                                return;
                            }
                            info = { tile: info };
                        }
            ...
            if (ctx.layer !== undefined) {
                ...
                if (tile && tile.effects) {
                    ...
                    if (effect) {
                        ...
                        didSomething = await GW.effect.fire(effect, map, x, y, ctx);
                        // cell.debug(" - spawned");
                        // }
                    }
                }
                ...
                for (let tile of this.tiles()) {
                    ...
                    if (effect) {
                        ...
                        // console.log(' - spawn event @%d,%d - %s', x, y, name);
                        didSomething =
                            (await GW.effect.fire(effect, map, x, y, ctx)) ||
                                didSomething;
                        // cell.debug(" - spawned");
                        if (didSomething) {
                            break;
