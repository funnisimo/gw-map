import { flag, config as config$1, color, range, grid, utils, make as make$6, data, message, events, random, sprite, path, fov, colors } from 'gw-utils';
...
async function spawn(activation, ctx = {}) {
    ...
    let item = null;
    if (feat.item && "item" in make$6) {
        item = make$6.item(feat.item);
        if (!item) {
...
class Tile$1 extends Entity {
    ...
    constructor(config) {
        ...
        // @ts-ignore
        this.flags.layer = flag.from(Layer, this.flags.layer, config.layerFlags || config.flags);
        // @ts-ignore
