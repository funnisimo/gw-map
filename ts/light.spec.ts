...
describe('light', () => {
    ...
    describe('updateLighting', () => {
        ...
        test('defaults to having white light', () => {
            expect(
                map.flags.map & Map.map.Flags.MAP_STABLE_GLOW_LIGHTS
            ).toBeTruthy();
            expect(
                map.flags.map & Map.map.Flags.MAP_STABLE_LIGHTS
            ).toBeTruthy();
            map.eachCell((cell) => {
                expect(cell.light).toEqual([100, 100, 100]);
                expect(cell.flags.cell & Map.cell.Flags.CELL_LIT).toBeFalsy();
                expect(cell.flags.cell & Map.cell.Flags.CELL_DARK).toBeFalsy();
                expect(
                    cell.flags.cell & Map.cell.Flags.LIGHT_CHANGED
                ).toBeFalsy();
            });
        });
        ...
        test('will return to default from stable glow lights', () => {
            ...
            map.eachCell((cell) => {
                expect(cell.light).toEqual([100, 100, 100]);
                expect(
                    cell.flags.cell & Map.cell.Flags.IS_IN_SHADOW
                ).toBeTruthy();
                expect(cell.flags.cell & Map.cell.Flags.CELL_LIT).toBeFalsy();
                expect(cell.flags.cell & Map.cell.Flags.CELL_DARK).toBeFalsy();
                expect(
        ...
        test('will add lights from tiles', () => {
            ...
            expect(map.flags.map & Map.map.Flags.MAP_STABLE_LIGHTS).toBeFalsy();
            expect(
                map.flags.map & Map.map.Flags.MAP_STABLE_GLOW_LIGHTS
            ).toBeFalsy();
