...
describe('spread type', () => {
    ...
    let fn: jest.MockedFunction<ACTION.ActionFn>;
    ...
    describe('compute spread map', () => {
        test('0:0', () => {
            ...
            expect(
                Spread.computeSpawnMap(effect.config, action, grid)
            ).toBeTruthy();

            expect(grid.count((v) => !!v)).toEqual(1);
            expect(grid[action.x][action.y]).toEqual(1);
        });

        test('0:0', () => {
            const effect = Spread.spread({
                grow: 0,
                decrement: 0,
                actions: fn,
            });
        ...
        test('100:100', () => {
            const effect = Spread.spread([100, 100, fn]);
        ...
        test('build in walls', () => {
            ...
            expect(
                effect.config.flags & Flags.Effect.E_BUILD_IN_WALLS
            ).toBeTruthy();
            expect(
                Spread.computeSpawnMap(effect.config, action, grid)
            ).toBeFalsy();
            ...
            // wall
            map.setTile(action.x, action.y, 'WALL');
        ...
        test('must touch walls', () => {
            ...
            expect(
                effect.config.flags & Flags.Effect.E_MUST_TOUCH_WALLS
            ).toBeTruthy();
            expect(
                Spread.computeSpawnMap(effect.config, action, grid)
            ).toBeFalsy();
            ...
            expect(grid.count(2)).toEqual(0);
            expect(grid.count(0)).toEqual(map.width * map.height);
            expect(grid[action.x][action.y]).toEqual(0);

            // next to a wall
            map.setTile(action.x, action.y, 'FLOOR', true);
            map.setTile(action.x, action.y - 1, 'WALL');
            expect(
                Spread.computeSpawnMap(effect.config, action, grid)
            ).toBeTruthy();
        ...
        test('no touch walls', () => {
            ...
            expect(
                effect.config.flags & Flags.Effect.E_NO_TOUCH_WALLS
            ).toBeTruthy();
        ...
        test('spread map - blocks effects', () => {
            ...
            map.setTile(action.x, action.y - 1, tile);
            map.setTile(action.x - 1, action.y, tile);

            expect(
                map
                    .cell(action.x, action.y - 1)
                    .hasEntityFlag(Flags.Entity.L_BLOCKS_EFFECTS)
            ).toBeTruthy();
            expect(
                map
                    .cell(action.x - 1, action.y)
                    .hasEntityFlag(Flags.Entity.L_BLOCKS_EFFECTS)
            ).toBeTruthy();

            let cell = map.cell(action.x - 1, action.y);
            expect(cell.blocksEffects()).toBeTruthy();
            cell = map.cell(action.x, action.y - 1);
            expect(cell.blocksEffects()).toBeTruthy();
            ...
            expect(grid[action.x][action.y]).toEqual(1);
            expect(grid[action.x - 1][action.y]).toEqual(0);
            expect(grid[action.x + 1][action.y]).toEqual(2);
            expect(grid[action.x][action.y - 1]).toEqual(0);
            expect(grid[action.x][action.y + 1]).toEqual(2);
        });
        ...
        test('50:0', () => {
            ...
            const effect = Spread.spread(50, 0, fn);
            action.x = 10;
            action.y = 10;
        ...
        test('100:10, matchTile: "DOOR"', () => {
            ...
            const effect = Spread.spread(100, 10, fn, { matchTile: 'DOOR' });
        ...
        test('50:10', () => {
            GWU.rng.random.seed(12345);
            const effect = Spread.spread(50, 10, fn);
            action.x = 10;
            action.y = 10;
    ...
    describe('fire', () => {
        test('tile and neighbors - string', () => {
            ...
            expect(map.cell(action.x, action.y).hasTile('WALL')).toBeFalsy();
            effect!(action);
        ...
        test('tile and neighbors - object', () => {
            ...
            expect(action.isSuccess()).toBeTruthy();
            expect(
                map.cell(action.originX, action.originY).hasTile('WALL')
            ).toBeTruthy();
            GWU.xy.eachNeighbor(
                action.originX,
                action.originY,
        ...
        test('can clear extra tiles from the cell', () => {
            ...
            map.setTile(action.x, action.y, 'BRIDGE');
