...
describe('visibility', () => {
    test('default - fov = off', () => {
        ...
        expect(map.flags.map & Map.map.Flags.MAP_CALC_FOV).toBeFalsy();
        expect(map.isVisible(3, 3)).toBeTruthy();
        expect(map.isRevealed(3, 3)).toBeTruthy();
    });
    ...
    test('fov on', () => {
        ...
        expect(map.flags.map & Map.map.Flags.MAP_CALC_FOV).toBeTruthy();
    ...
    test('fov off - forces reveal+visible', () => {
        const map: Map.map.Map = Map.map.make(10, 10, {
            tile: 'FLOOR',
            wall: 'WALL',
            fov: false, // automagically reveals and makes visible whole map
        });
        expect(map.isVisible(3, 3)).toBeTruthy();
        expect(map.isRevealed(3, 3)).toBeTruthy();
        expect(map.flags.map & Map.map.Flags.MAP_CALC_FOV).toBeFalsy();
